...
export function exportHajjiExcel({ title, customers }: HajjiReportData) {
  ...
  rows.push([]);
  rows.push(["Hasan Travels"]);
  rows.push(["Phone: +880 1886-999465 | Email: alrawshainternational@gmail.com"]);
...
export function exportExcel({ title, columns, rows, summary }: ReportData) {
  ...
  wsData.push([]);
  wsData.push(["Hasan Travels"]);
  wsData.push(["Phone: +880 1886-999465 | Email: alrawshainternational@gmail.com"]);
