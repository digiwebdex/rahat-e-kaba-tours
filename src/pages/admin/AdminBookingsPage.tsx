...
export default function AdminBookingsPage() {
  ...
  const startEdit = async (b: any) => {
    ...
    setEditForm({
      ...
      moallem_id: b.moallem_id || (moallems.find((m: any) => m.name === "Hasan Travels")?.id || ""),
  ...
  const handleStatusChange = async () => {
    ...
    if (booking?.user_id && oldStatus !== statusChangeVal) {
      try {
        ...
        const smsMsg = `Hasan Travels: Dear ${booking.guest_name || "Customer"}, your booking (${booking.tracking_id}) status has been updated to "${newLabel}". Package: ${pkgName}. For queries: 01711-999910`;
        ...
        const emailHtml = `
          <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;">
            <div style="background:#C5A55A;padding:15px;text-align:center;border-radius:8px 8px 0 0;">
              <h2 style="color:#fff;margin:0;">Hasan Travels</h2>
            </div>
            <div style="border:1px solid #e5e5e5;border-top:0;padding:20px;border-radius:0 0 8px 8px;">
              ...
              <p style="color:#888;font-size:12px;margin-top:20px;">Thank you for choosing Hasan Travels.</p>
            </div>
          </div>`;
